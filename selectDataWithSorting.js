...
import fs from "fs";
const selectDataSqlite = (obj, sort) => {
  var start = new Date();
  ...
  if (sort.sort == "ASC") str2 = " ORDER BY " + sort.column + " ASC";
  else if (sort.sort == "DESC") str2 = " ORDER BY " + sort.column + " DESC";
  else str2 = "";
  ...
  db4.each(
    ...
    function () {
      ...
      fs.writeFile(
        "./badania/selectDataWidthSortSqlite.txt",
        end + "\n",
        { flag: "a+" },
        (err) => {
          if (err) {
              ...
              if (doc != null) {
                ...
                const DocumentObject =
                  documentsObj.length != 0
                    ? JSON.parse(
                        `{"person_id":"${docs[i]._id}", ${documentStr}}`
                      )
                    : JSON.parse(`{"person_id":"${docs[i]._id}"}`);
                ...
                db3.documents.findOne(DocumentObject, function (err, doc) {
                  ...
                  if (i == docs.length - 1) {
                    var end = new Date() - start;
                    fs.writeFile(
                      "./badania/selectDataWidthSortNeDB.txt",
                      end + "\n",
                      { flag: "a+" },
                      (err) => {
                        if (err) {
...
async function selectDataLowDB(obj, sort) {
  var start = new Date();
  ...
  var end = new Date() - start;
  fs.writeFile(
    "./badania/selectDataWidthSortLowDB.txt",
    end + "\n",
    { flag: "a+" },
    (err) => {
      if (err) {
...
async function selectDataLevelDB(obj) {
  ...
  var result = Array.from(Array(column.length).keys()).sort((a, b) =>
    column[a] > column[b] ? -1 : (column[b] > column[a]) | 0
  );

  for (let i = 0; i < result.length; i++) {
    if (
      id[result[i]] != undefined &&
      titleNames[result[i]] != undefined &&
      firstNames[result[i]] != undefined &&
      lastNames[result[i]] != undefined &&
      genders[result[i]] != undefined &&
      streetNames[result[i]] != undefined &&
      streetNumbers[result[i]] != undefined &&
      cities[result[i]] != undefined &&
      states[result[i]] != undefined &&
      countries[result[i]] != undefined &&
      postCodes[result[i]] != undefined &&
      coordLatitudes[result[i]] != undefined &&
      coordLongitudes[result[i]] != undefined &&
      offsetTimeZones[result[i]] != undefined &&
      descriptionsTimeZone[result[i]] != undefined &&
      nationals[result[i]] != undefined &&
      cells[result[i]] != undefined &&
      phones[result[i]] != undefined &&
      emails[result[i]] != undefined &&
      pictures[result[i]] != undefined &&
      datesOfBirth[result[i]] != undefined &&
      ages[result[i]] != undefined &&
      documentNames[result[i]] != undefined &&
      documentValues[result[i]] != undefined &&
      userNames[result[i]] != undefined &&
      passwords[result[i]] != undefined &&
      registredDates[result[i]] != undefined &&
      registredYears[result[i]] != undefined
    ) {
      // console.log(
      //   "id: " +
      //     id[result[i]] +
  ...
  var end = new Date() - start;
  fs.writeFile(
    "./badania/selectDataWidthSortLevelDB.txt",
    end + "\n",
    { flag: "a+" },
    (err) => {
      if (err) {
...
const obj = { gender: "female", titleName: "Miss" };

/////// SORTING VALUE ///////////
// Sqlite {column: "age", sort: "ASC"}
// NEDB {age: -1}
// LowDB {variable:"dateOfBirth.age", sort: "desc"}

selectDataSqlite(obj, { column: "firstname", sort: "DESC" });
selectDataNedb(obj, { firstName: -1 });
selectDataLowDB(obj, { variable: "name.first", sort: "desc" });
