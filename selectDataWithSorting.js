...
const selectDataSqlite = (obj, sort) => {
    ...
    db4.each(`SELECT * FROM persons join addresses ON persons.id=addresses.id join documents ON persons.id=documents.person_id join accounts ON persons.id=accounts.person_id ${str+str2}`, function(err, row) {
...
const selectDataNedb = (obj, sort) => {
    ...
    db3.persons.find(PersonObject).sort(sort).exec(function (err, docs) {
        let account = [];
        let address = [];
        let document = [];
...
async function selectDataLevelDB(obj){
    ...
    for(let i =0; i<result.length; i++){
        ...
        if(id[result[i]]!=undefined && titleNames[result[i]]!=undefined && firstNames[result[i]]!=undefined && lastNames[result[i]]!=undefined && genders[result[i]]!=undefined && streetNames[result[i]]!=undefined && streetNumbers[result[i]]!=undefined && cities[result[i]]!=undefined && states[result[i]]!=undefined && countries[result[i]]!=undefined && postCodes[result[i]]!=undefined && coordLatitudes[result[i]]!=undefined && coordLongitudes[result[i]]!=undefined && offsetTimeZones[result[i]]!=undefined && descriptionsTimeZone[result[i]]!=undefined && nationals[result[i]]!=undefined && cells[result[i]]!=undefined && phones[result[i]]!=undefined && emails[result[i]]!=undefined && pictures[result[i]]!=undefined && datesOfBirth[result[i]]!=undefined && ages[result[i]]!=undefined && documentNames[result[i]]!=undefined &&  documentValues[result[i]]!=undefined && userNames[result[i]]!=undefined && passwords[result[i]]!=undefined && registredDates[result[i]]!=undefined && registredYears[result[i]]!=undefined){
...
// Leveldb  zmienic zmienną column na odpoweidnia kolumne
