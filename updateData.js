...
const updateDataSqlite = (searchObj, updateObj) => {
  ...
  for (let i = 0; i < columnsUpdate.length; i++) {
    if (personsList.includes(columnsUpdate[i])) {
      const temp = isNaN(updateObj[columnsUpdate[i]])
        ? '"' + updateObj[columnsUpdate[i]] + '"'
        : updateObj[columnsUpdate[i]];
  ...
  for (let i = 0; i < updateDataAddresses.length; i++) {
    ...
    addressUpdateStr +=
      i < updateDataAddresses.length - 1
        ? tmp[0] + "=" + tmp[1] + ", "
        : tmp[0] + "=" + tmp[1] + "";
  }
  ...
  for (let i = 0; i < updateDataDocuments.length; i++) {
    ...
    documentUpdateStr +=
      i < updateDataDocuments.length - 1
        ? tmp[0] + "=" + tmp[1] + ", "
        : tmp[0] + "=" + tmp[1] + "";
  }
  ...
  if (personUpdateStr != "" && personStr != "")
    db4.run(
      "UPDATE persons SET " + personUpdateStr + " WHERE " + personStr,
      () => {
        var end = new Date() - start;
        fs.writeFile(
          "./badania/updateSqlite.txt",
          end + "\n",
          { flag: "a+" },
          (err) => {
            if (err) {
  ...
  if (accountUpdateStr != "" && accountStr != "")
    db4.run(
      "UPDATE accounts SET " + accountUpdateStr + " WHERE " + accountStr,
      () => {
        var end = new Date() - start;
        fs.writeFile(
          "./badania/updateSqlite.txt",
  ...
  if (addressUpdateStr != "" && addressStr != "")
    db4.run(
      "UPDATE addresses SET " + addressUpdateStr + " WHERE " + addressStr,
      () => {
        var end = new Date() - start;
        fs.writeFile(
          "./badania/updateSqlite.txt",
  ...
  if (documentUpdateStr != "" && documentStr != "")
    db4.run(
      "UPDATE documents SET " + documentUpdateStr + " WHERE " + documentStr,
      () => {
        var end = new Date() - start;
        fs.writeFile(
          "./badania/updateSqlite.txt",
...
const updateDataNedb = (searchObj, updateObj) => {
  ...
  for (let i = 0; i < personsDataObj.length; i++) {
    i == personsDataObj.length - 1
      ? (personUpdateStr += personsDataObj[i])
      : (personUpdateStr += personsDataObj[i] + ",");
  }
  for (let i = 0; i < addressesDataObj.length; i++) {
    i == addressesDataObj.length - 1
      ? (addressUpdateStr += addressesDataObj[i])
      : (addressUpdateStr += addressesDataObj[i] + ",");
  }
  for (let i = 0; i < accountsDataObj.length; i++) {
    i == accountsDataObj.length - 1
      ? (accountUpdateStr += accountsDataObj[i])
      : (accountUpdateStr += accountsDataObj[i] + ",");
  }
  for (let i = 0; i < documentsDataObj.length; i++) {
    i == documentsDataObj.length - 1
      ? (documentUpdateStr += documentsDataObj[i])
      : (documentUpdateStr += documentsDataObj[i] + ",");
  }
  ...
  if (personStr != "" && personUpdateStr != "") {
    ...
    db3.persons.update(
      PersonObject,
      { $set: PersonChangeObject },
      { multi: true },
      function (err, numReplaced) {
        db3.persons.persistence.compactDatafile();
        var end = new Date() - start;
        fs.writeFile(
          "./badania/updateNeDB.txt",
          end + "\n",
          { flag: "a+" },
          (err) => {
            if (err) {
...
async function updateDataLowDB(searchObj, updateObj) {
  ...
  const SearchObj = new User(
    ...
    searchObj.firstName,
    searchObj.lastName,
    searchObj.streetName,
    searchObj.streetNumber,
    searchObj.city,
    searchObj.state,
    searchObj.country,
    searchObj.postCode,
    searchObj.coordLatitude,
    searchObj.coordLongitude,
    searchObj.offsetTimeZone,
    searchObj.descriptionTimeZone,
    searchObj.email,
    searchObj.uuid,
    searchObj.userName,
    searchObj.password,
    searchObj.dateOfBirth,
    searchObj.age,
    searchObj.registredDate,
    searchObj.registredYear,
    searchObj.phone,
    searchObj.cell,
    searchObj.documentName,
    searchObj.documentValue,
    searchObj.picture,
    searchObj.national
  );

  const UpdateObj = new User(
    updateObj.gender,
    updateObj.titleName,
    updateObj.firstName,
    updateObj.lastName,
    updateObj.streetName,
    updateObj.streetNumber,
    updateObj.city,
    updateObj.state,
    updateObj.country,
    updateObj.postCode,
    updateObj.coordLatitude,
    updateObj.coordLongitude,
    updateObj.offsetTimeZone,
    updateObj.descriptionTimeZone,
    updateObj.email,
    updateObj.uuid,
    updateObj.userName,
    updateObj.password,
    updateObj.dateOfBirth,
    updateObj.age,
    updateObj.registredDate,
    updateObj.registredYear,
    updateObj.phone,
    updateObj.cell,
    updateObj.documentName,
    updateObj.documentValue,
    updateObj.picture,
    updateObj.national
  );
  ...
  db2.chain
    ...
    .filter(SearchObj)
    .each((user) => {
      if ("gender" in UpdateObj) user.gender = UpdateObj.gender;

      if ("name" in UpdateObj) {
        if ("title" in UpdateObj.name) user.name.title = UpdateObj.name.title;
        if ("first" in UpdateObj.name) user.name.first = UpdateObj.name.first;
        if ("last" in UpdateObj.name) user.name.last = UpdateObj.name.last;
      }
        ...
        if ("coordinates" in UpdateObj.location) {
          if ("latitude" in UpdateObj.location.coordinates)
            user.location.coordinates.latitude =
              UpdateObj.location.coordinates.latitude;
          if ("longitude" in UpdateObj.location.coordinates)
            user.location.coordinates.longitude =
              UpdateObj.location.coordinates.longitude;
        }
        ...
        if ("timezone" in UpdateObj.location) {
          ...
          if ("description" in UpdateObj.location.timezone)
            user.location.timezone.description =
              UpdateObj.location.timezone.description;
        }
      }

      if ("email" in UpdateObj) user.email = UpdateObj.email;
      if ("login" in UpdateObj) {
        if ("uuid" in UpdateObj.login) user.login.uuid = UpdateObj.login.uuid;
      ...
      if ("phone" in UpdateObj) user.phone = UpdateObj.phone;

      if ("cell" in UpdateObj) user.cell = UpdateObj.cell;

      if ("id" in UpdateObj) {
        if ("name" in UpdateObj.id) user.id.name = UpdateObj.id.name;
        if ("value" in UpdateObj.id) user.id.value = UpdateObj.id.value;
      }

      if ("picture" in UpdateObj) user.picture = UpdateObj.picture;
      if ("national" in UpdateObj) user.national = UpdateObj.national;
    })
    .value();
...
async function selectDataLevelDB(SearchObj, UpdateObj) {
  ...
  for await (const [key, value] of db.iterator()) {
    ...
    if (str.split(":")[0] == "coordLatitude") {
      if (
        SearchObj.coordLatitude == value + "" ||
        SearchObj.coordLatitude == undefined
      )
    ...
    if (str.split(":")[0] == "coordLongitude") {
      if (
        SearchObj.coordLongitude == value + "" ||
        SearchObj.coordLongitude == undefined
      )
    ...
    if (str.split(":")[0] == "dateOfBirth") {
      if (
        SearchObj.dateOfBirth == value + "" ||
        SearchObj.dateOfBirth == undefined
      )
    ...
    if (str.split(":")[0] == "descriptionTimeZone") {
      if (
        SearchObj.descriptionTimeZone == value + "" ||
        SearchObj.descriptionsTimeZone == undefined
      )
    ...
    if (str.split(":")[0] == "documentName") {
      if (
        SearchObj.documentName == value + "" ||
        SearchObj.documentNames == undefined
      )
    ...
    if (str.split(":")[0] == "documentValue") {
      if (
        SearchObj.documentValue == value + "" ||
        SearchObj.documentValue == undefined
      )
    ...
    if (str.split(":")[0] == "offsetTimeZone") {
      if (
        SearchObj.offsetTimeZone == value + "" ||
        SearchObj.offsetTimeZone == undefined
      )
    ...
    if (str.split(":")[0] == "registredDate") {
      if (
        SearchObj.registredDate == value + "" ||
        SearchObj.registredDate == undefined
      )
    ...
    if (str.split(":")[0] == "registredYear") {
      if (
        SearchObj.registredYear == value + "" ||
        SearchObj.registredYear == undefined
      )
    ...
    if (str.split(":")[0] == "streetName") {
      if (
        SearchObj.streetName == value + "" ||
        SearchObj.streetName == undefined
      )
    ...
    if (str.split(":")[0] == "streetNumber") {
      if (
        SearchObj.streetNumber == value + "" ||
        SearchObj.streetNumber == undefined
      )
...
updateDataSqlite({ gender: "female" }, { titleName: "woman" });
updateDataNedb({ gender: "female" }, { titleName: "woman" });
updateDataLowDB({ gender: "female" }, { titleName: "woman" });
selectDataLevelDB({ gender: "female" }, { titleName: "woman" });
